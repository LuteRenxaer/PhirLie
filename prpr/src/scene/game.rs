...
use crate::{
    ...
    ext::{format_number, parse_time, screen_aspect, semi_black, semi_white, RectExt, SafeTexture, ScaleType},
    fs::FileSystem,
...
impl GameScene {
            ...
            if self.judge.combo() >= 3 || is_autoplay {
                if legacy_aui {
                    ...
                    if is_autoplay {
                        ...
                        let y_pos = top + eps * 2. - (1. - p) * 0.4 + text_r.center().y;
                        self.chart.with_element(ui, res, UIElement::ComboNumber, None, (0., y_pos), |ui, c| {
                            ui.text(text)
                                .pos(0., y_pos)
                                .anchor(0.5, 0.5)
                                .size(size)
    ...
    fn overlay_ui(&mut self, ui: &mut Ui, tm: &mut TimeManager) -> Result<()> {
        ...
        if tm.paused() {
            ...
            for (i, pos) in btn_positions.iter().enumerate() {
                let x = (s * 2. + w) * (*pos) as f32;
                let r = Rect::new(x - s, -s + o, s * 2., s * 2.);
                let is_disabled = (i == 1 && no_retry) || (i == 2 && self.dead);
                let color = if is_disabled {
                    semi_white(res.alpha * 0.3)
                } else {
                ...
                if !is_disabled {
                    ui.text(btn_icons[i].1)
                        ...
                        .anchor(0.5, 0.)
                        .size(0.3)
                        .color(semi_white(0.7 * res.alpha))
                        .draw();
                }
            }
...
impl Scene for GameScene {
    ...
    fn render(&mut self, tm: &mut TimeManager, ui: &mut Ui) -> Result<()> {
        ...
        if !watermark_text.is_empty() {
            let wm_color = Color::new(1.0, 1.0, 1.0, 0.3);
            let text_size = 28.0;
            let text_measure = measure_text(&watermark_text, None, text_size as u16, 1.0);
            let x = (screen_width() - text_measure.width) / 2.0;
            let y = screen_height() - 30.0;
            draw_text(&watermark_text, x, y, text_size, wm_color);
        }
